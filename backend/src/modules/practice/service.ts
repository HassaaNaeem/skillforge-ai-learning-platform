...
import { enqueueEvaluateAnswer } from '../../queues/evaluateAnswer.js';
...
export async function submitAnswer(
  ...
  if (!practiceSession) throw new AppError(404, 'Practice session not found');
  if (practiceSession.userId !== userId)
    throw new AppError(403, 'You are not authorized to submit answer for this practice session');
  ...
  if (question.topicId !== practiceSession.topicId)
    throw new AppError(400, 'Question does not belong to the practice session topic');
